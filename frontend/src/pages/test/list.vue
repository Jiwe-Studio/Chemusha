            ...
            <q-card bordered  flat  class="q-pa-md q-mb-md no-border-radius shadow-1">
                <div class="container-fluid">
                    <div class="row justify-between q-col-gutter-md">
                        <div class="col ">
                            <div class="text-h6 text-bold record-title">Test</div>
                        </div>
                        <div class="col-md-auto col-12 ">
                            <q-btn    :rounded="false"  size=""  color="primary"  no-caps  unelevated    to="/test/add" class="full-width">
                    ...
                    <div class="col comp-grid">
                        <div>
                            <template v-if="showBreadcrumbs">
                                <q-breadcrumbs class="q-pa-md" v-if="searchText || fieldName">
                                    <template v-if="fieldName">
                                        <q-breadcrumbs-el v-if="$route.query.tag" icon="las la-angle-left" class="text-capitalize" :label="$route.query.tag" to="/test"></q-breadcrumbs-el>
                                        <q-breadcrumbs-el v-else icon="las la-angle-left" class="text-capitalize" :label="fieldName|readable" to="/test"></q-breadcrumbs-el>
                            ...
                            <div class="relative-position">
                                ...
                                :bordered="false"
                                :columns="$menus.TestTableHeaderItems" 
                                :data="records"
                                binary-state-sort
                                ...
                                <template v-slot:body="props">
                                    <q-tr :props="props">
                                        ...
                                        <q-td key="id" :props="props">
                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="'/test/view/' + props.row.id">{{ props.row.id }}</q-btn>
                                        </q-td>
                                        <q-td key="post" :props="props">
                                            <div v-html="props.row.post"></div>
                                        </q-td>
                                        <q-td key="btnactions" :props="props">
                                            <q-btn icon="las la-eye"    :rounded="false"  no-caps  unelevated   size="sm" padding="xs" color="accent" title="View Record" :to="`/test/view/${ props.row.id }`">
                                            </q-btn>
                                            <q-btn icon="las la-edit"    :rounded="false"  no-caps  unelevated   size="sm" padding="xs" color="accent" title="Edit This Record" :to="`/test/edit/${ props.row.id }`" class="q-mx-xs">
                                            </q-btn>
...
<script>
	...
	export default {
		...
		computed: {
			...
			records: {
				get: function () {
					return this.$store.getters["test/records"];
				},
				set: function (value) {
					this.$store.commit("test/setRecords", value);
				},
			},
		},
		meta () {
		...
		methods: {
			...mapActions("test", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;
					let url = this.apiUrl;
