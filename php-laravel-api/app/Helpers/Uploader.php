...
class Uploader
{
	...
	function upload(Request $request)
	{
		...
		if (!$this->request->hasFile("file")) {
			$this->errors[] = "no file selected for upload";
		}
		...
		if ($validator->fails()) {
			$this->errors[] = validationErrorsToString($validator->errors());
		}
		...
		if (count($files) > $this->fileLimit) {
			$this->errors[] = "Maximum number of files ({$this->fileLimit}) exceeded";
		}
